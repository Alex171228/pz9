...
1. Склонировать репозиторий и перейти в папку проекта:
   ```bash
   git clone https://github.com/Alex171228/Pz9
   cd pz9
    ```
2. Создать файл .env:
      ```bash
...
Обычные хэш-функции (например, SHA-256) недостаточно безопасны для хранения паролей, так как позволяют выполнять быстрый перебор с использованием GPU или радужных таблиц. 
bcrypt специально разработан для защиты паролей: он медленный по вычислениям и настраиваемый по сложности (cost), что делает перебор практически неэффективным. Он автоматически  добавляет соль, поэтому одинаковые пароли не имеют одинаковых хэшей. 
